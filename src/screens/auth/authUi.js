...
// Header decoration shared by Login + OTP screens: two-tone overlapping
// circles top-right, a small circle top-left, then the "reconnct" wordmark
// and tagline underneath — matches the Figma auth-screen reference exactly.
export function AuthHeader() {
  const insets = useSafeAreaInsets();
  return (
    <View style={[authStyles.decorWrap, { paddingTop: insets.top + 18 }]}>
      <View style={authStyles.circleBig} />
      <View style={authStyles.circleBigInner} />
      <View style={authStyles.circleSmall} />
      <Text style={authStyles.logoText}>reconnct</Text>
      <Text style={authStyles.tagline}>with your loved ones</Text>
    </View>
  );
}

export function AuthField({ icon, ...inputProps }) {
  return (
...
export function AuthButton({ label, active, loading, onPress }) {
  return (
    <TouchableOpacity
      style={[authStyles.btn, active ? authStyles.btnActive : authStyles.btnDisabled]}
      activeOpacity={0.85}
      onPress={onPress}
...
export const authStyles = StyleSheet.create({
  decorWrap: { paddingHorizontal: 24, paddingBottom: 24, overflow: 'hidden' },
  circleBig: { position: 'absolute', top: -55, right: -45, width: 170, height: 170, borderRadius: 85, backgroundColor: colors.brand },
  circleBigInner: { position: 'absolute', top: 4, right: 26, width: 84, height: 84, borderRadius: 42, backgroundColor: colors.brandDark },
  circleSmall: { position: 'absolute', top: 20, left: 22, width: 34, height: 34, borderRadius: 17, backgroundColor: colors.brandDark },
  logoText: { fontSize: 30, fontWeight: '800', color: colors.brandDark, marginTop: 66 },
  tagline: { fontSize: font.small, color: colors.inkMuted, marginTop: 2 },

  fieldWrap: {
    flexDirection: 'row', alignItems: 'center', gap: 8,
    backgroundColor: 'rgba(231,231,235,0.5)',
    borderWidth: 1, borderColor: 'rgba(136,136,153,0.2)',
    borderRadius: 8, height: 44, paddingHorizontal: 10,
  },
  fieldInput: { flex: 1, fontSize: font.small, color: colors.ink, paddingVertical: 0 },

  btn: { height: 48, borderRadius: 8, alignItems: 'center', justifyContent: 'center', marginTop: 16 },
